...
contract Escrow {
    ...
    bool public conditionPasssed = false;
    ...
    function updateConditionStatus(bool _passed) public onlyInspector {
        conditionPasssed = _passed;
    }
    ...
    // Finalize Sale
    // -> Require condition status 
    // -> Require sale to be authorized
    // -> Require funds to be correct amount
    // -> Transfer NFT to buyer
    // -> Transfer Funds to Seller
    function finalizeSale() public {
        require(conditionPasssed);
    ...
    // -> if condition status is not approved, then refund, otherwise send to seller
    function cancelSale() public {
        if(conditionPasssed == false) {
